...
private:
  ...
  uint16_t applyGain(const uint16_t& strip,const uint8_t& adc );
...
applyGain(const uint16_t& strip,const uint8_t& adc ) {
...
DEFINE_ANOTHER_FWK_MODULE(SiStripClusterToDigiProducer);
