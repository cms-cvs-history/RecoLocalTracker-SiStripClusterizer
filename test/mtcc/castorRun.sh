...
#Create input file list
    ...
    for runs in $runList
      ...
      while [ ${firstRun} -le ${lastRun} ]
	do
	castorRun=`nsls ${castor_path} | grep ${firstRun}`
	for file in `nsls ${castor_path}/${castorRun}/A 2> /dev/null`
	  do
	  inputfilenames="${inputfilenames},\"castor:${castor_path}/${castorRun}/A/$file\""
	done
	let firstRun++
      done      
    done
...
export castor_path=/castor/cern.ch/cms/MTCC/data
